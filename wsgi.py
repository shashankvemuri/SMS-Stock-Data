...
def get_sell_rating(ticker):
    try:
        ...
        condition_19 = df["High"][-1] > upperband[-1] and df["High"][-2] > upperband[-2] and df["High"][-3] > upperband[-3] and df["High"][-4] > upperband[-4]

        # Condition 20: Price > 50.2 BBANDS (3x)
        condition_20 = df["Adj Close"][-1] > upperband[-1] and df["Adj Close"][-2] > upperband[-2] and df["Adj Close"][-3] > upperband[-3] and df["Adj Close"][-4] > upperband[-4]

        # Condition 21: Low > 50.2 BBANDS (3x)
        condition_21 = (df["Low"][-1] > upperband[-1] and df["Low"][-2] > upperband[-2] and df["Low"][-3] > upperband[-3]) or df["Low"][-1] > 1.05 * upperband[-1]
